...
//Handle multiple socket connections with select and fd_set on Linux
        ...
        struct sockaddr_in address, addressClient;
        int condicion;
    ...
    //bind the socket to localhost port 8080
        ...
        while (TRUE) {
            ...
            //then its an incoming connection9
            ...
            for (i = 0; i < max_clients; i++) {
                ...
                if (FD_ISSET(sd, &readfds)) {
                    ...
                    if ((readFromClient(sd,buffer) == 0)) {
                        ...
                        cout << "envia o recibe: " << endl;
                        cin >> condicion;

                        if (condicion == 1) {
                            clientListener = new Client();
                            clientListener.run();
                            clientListener.sendToServer(sd,"HellouFROMFabi");
                            //char * mensajeX = "HELLOU";
                            sendToClient(sd, "HELLOU");
                            printf("message sent\n");
